...
local GblRelayState = {"OFF", "OFF", "OFF", "OFF"}
	...
	eco.run(function(name)		
		while true do 		
			...
			if cmdChar=="S" then			
				...
				local SensorNum = line:sub(2)
				if verbose(2) then  
					print("Reading sensors" )
					print("linenumber", tonumber(SensorNum) )
				end
				if SensorNum=="-1" then
					ShapeAndSendData()
				else				
					ShapeAndSendData(tonumber(SensorNum) )				
				end
...
function SetRelay(cmdLine)
	local RelayNum, Cmnd = cmdLine:match("([^_]+)_([^_]+)")
	RelayNum=tonumber(RelayNum)
	if Cmnd=="on" then
		GblRelayState[RelayNum]="ON"
	elseif Cmnd=="off" then
		GblRelayState[RelayNum]="OFF"
	end	
	if verbose(1) then 
		print("RelayNum: ",RelayNum)
		print("Cmnd: ",Cmnd) 					
		print("GblRelayState[" .. RelayNum .. "]: " .. GblRelayState[RelayNum])
	end
